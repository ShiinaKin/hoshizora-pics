...
import io.sakurasou.hoshizora.exception.controller.access.SignupNotAllowedException
import io.sakurasou.hoshizora.exception.service.user.UserDeleteFailedException
import io.sakurasou.hoshizora.exception.service.user.UserInsertFailedException
import io.sakurasou.hoshizora.exception.service.user.UserNotFoundException
import io.sakurasou.hoshizora.exception.service.user.UserUpdateFailedException
...
import io.sakurasou.hoshizora.model.dto.ImageCountAndTotalSizeDTO
...
class UserServiceTest {
    ...
    @Test
    fun `signup should throw SignupNotAllowedException if system not allowed signup`(): Unit =
        runBlocking {
            val userInsertRequest =
                UserInsertRequest(
                    username = "testUser",
    ...
    @Test
    fun `signup should be successful`() =
        runBlocking {
            val userInsertRequest =
                UserInsertRequest(
                    username = "testUser",
    ...
    @Test
    fun `saveUserManually should be successful`() =
        runBlocking {
            val userManageInsertRequest =
    ...
    @Test
    fun `saveUserManually should throw UserInsertFailedException`() =
        runBlocking {
            val userManageInsertRequest =
                UserManageInsertRequest(
                    groupId = 1,
    ...
    @Test
    fun `deleteUser should delete user successfully when user exists`() =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `deleteUser should throw UserDeleteFailedException when user not found`() =
        runBlocking {
            val userId = 1L
            every { imageDao.deleteImageByUserId(userId) } returns 1
    ...
    @Test
    fun `patchSelf should update user successfully when all data is valid`() =
        runBlocking {
            val userId = 1L
            val oldUserInfo =
                User(
    ...
    @Test
    fun `patchSelf should throw UserNotFoundException if user does not exist`(): Unit =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `patchSelf should throw UserUpdateFailedException if user does not exist`(): Unit =
        runBlocking {
            val userId = 1L
            val patchRequest = UserSelfPatchRequest()
            val oldUser =
    ...
    @Test
    fun `patchSelf should retain old password if new password is not provided`() =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `patchUser should update user successfully with all new data`() =
        runBlocking {
            val userId = 1L
            val oldUserInfo =
                User(
    ...
    @Test
    fun `patchUser should throw UserNotFoundException if user does not exist`(): Unit =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `patchUser should throw UserUpdateFailedException if user does not exist`(): Unit =
        runBlocking {
            val userId = 1L
            val oldUserInfo =
                User(
    ...
    @Test
    fun `patchUser should retain old password if new password is not provided`() =
        runBlocking {
            val userId = 1L
            val oldPassword = "oldPassword"
            val oldUserInfo =
    ...
    @Test
    fun `banUser should update ban status to true when user exists`() =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `banUser should throw UserUpdateFailedException when user does not exist`(): Unit =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `unbanUser should update ban status to false when user exists`() =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `unbanUser should throw UserUpdateFailedException when user does not exist`(): Unit =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `fetchUser should return valid UserVO when user exists and all related data is available`() =
        runBlocking {
            val userId = 1L
            val user =
                User(
    ...
    @Test
    fun `fetchUser should throw UserNotFoundException when user does not exist`(): Unit =
        runBlocking {
            val userId = 1L
