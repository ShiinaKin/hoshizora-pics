...
import io.sakurasou.hoshizora.exception.service.album.AlbumAccessDeniedException
import io.sakurasou.hoshizora.exception.service.album.AlbumDeleteFailedException
import io.sakurasou.hoshizora.exception.service.album.AlbumInsertFailedException
import io.sakurasou.hoshizora.exception.service.album.AlbumUpdateFailedException
...
class AlbumServiceTest {
    ...
    @Test
    fun `saveSelf should insert album correctly`() =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `saveSelf should throw AlbumInsertFailedException when save fails`() =
        runBlocking {
            val userId = 1L
            val request = AlbumSelfInsertRequest(name = "My Album", description = "Test Album")
            val expectedDTO =
    ...
    @Test
    fun `deleteSelf should delete album correctly when user is owner`() =
        runBlocking {
            val userId = 1L
            val albumId = 2L
            val album =
    ...
    @Test
    fun `deleteSelf should throw AlbumDeleteFailedException when album does not exist`() =
        runBlocking {
            val userId = 1L
            val albumId = 2L
    ...
    @Test
    fun `patchSelf should update album correctly when user is owner and data is valid`() =
        runBlocking {
            val userId = 1L
            val albumId = 2L
            val album =
    ...
    @Test
    fun `patchSelf should throw AlbumAccessDeniedException when user is not owner`() =
        runBlocking {
            val userId = 1L
    ...
    @Test
    fun `fetchSelf should return correct AlbumVO when user is owner`() =
        runBlocking {
            val userId = 1L
            val albumId = 2L
            val album =
    ...
    @Test
    fun `fetchSelf should throw AlbumAccessDeniedException when user is not owner`(): Unit =
        runBlocking {
            val userId = 1L
            val anotherUserId = 2L
            val albumId = 3L
