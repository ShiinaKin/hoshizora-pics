...
fun Application.configureCache(
    ...
    install(Cache) {
        if (host != "disabled") {
            redisProvider(host, port.toInt(), expireTime = 30.minutes)
        } else {
            memoryProvider(15.minutes)
        }
    }
}
...
val Cache =
    createApplicationPlugin("Cache", ::CacheConfig) {
...
val CacheRoutePlugin =
    createRouteScopedPlugin("CacheRoutePlugin", ::RouteCachePluginConfig) {
        val provider =
            application.attributes.getOrNull(AttributeKey<CacheProvider>("CacheProvider"))
        ...
        fun buildKey(call: ApplicationCall) =
            if (pluginConfig.specifyQueryParams.isEmpty()) {
                call.request.uri
            } else {
                pluginConfig.specifyQueryParams.joinToString("&") { "$it=${call.request.queryParameters[it]}" }.let {
